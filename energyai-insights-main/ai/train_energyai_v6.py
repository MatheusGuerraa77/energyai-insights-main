...
print("Carregando dataset EnergyAI V6...")
...
print("Criando variáveis inteligentes V6...")
...
lags = [
    1, 2, 3, 6, 12, 24, 48, 72, 96, 120, 144,
    168, 240, 336, 504, 672
]
...
janelas = [3, 6, 12, 24, 48, 72, 96, 168, 336, 504, 672]
...
for nome_horizonte, passos in horizontes.items():
    ...
    df_h[f"media_futura_referencia_{passos}h"] = (
        df_h["nat_demand"].shift(passos).rolling(passos).mean()
    )

    features_h = features + [
        f"lag_horizonte_{passos}h",
        f"diff_horizonte_{passos}h",
        f"media_futura_referencia_{passos}h",
    ]
    ...
    print(f"\nCalculando Ensemble V6 para {nome_horizonte}...")
    ...
    if best_model is not None and best_result["modelo"] == "HistGradientBoosting":
        joblib.dump(
            best_model,
            f"{MODELS_DIR}/best_model_{nome_horizonte}_v6.pkl",
            compress=3,
        )
    else:
        print("Modelo não salvo para evitar arquivo .pkl gigante.")

    joblib.dump(features_h, f"{MODELS_DIR}/features_{nome_horizonte}_v6.pkl", compress=3)
    ...
    plt.title(f"EnergyAI V6 - Previsão de Demanda - {nome_horizonte}")
    plt.xlabel("Amostras")
    plt.ylabel("Demanda elétrica")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{DOCS_DIR}/grafico_energyai_v6_{nome_horizonte}.png", dpi=200)
    plt.close()
...
with open(f"{DOCS_DIR}/metricas.txt", "w", encoding="utf-8") as file:
    file.write("MÉTRICAS DOS MODELOS ENERGYAI V6\n")
    file.write("================================\n\n")
