...
print("Carregando dataset EnergyAI V7 Híbrido...")
...
df["pico_hora"] = df["hour"].isin([17, 18, 19, 20, 21]).astype(int)
df["manha"] = df["hour"].between(6, 11).astype(int)
df["tarde"] = df["hour"].between(12, 17).astype(int)
df["noite"] = df["hour"].between(18, 23).astype(int)
df["madrugada"] = df["hour"].between(0, 5).astype(int)
...
janelas = [3, 6, 9, 10, 11, 12, 24, 48, 72, 96, 168, 336, 504, 672]
...
df["mesma_hora_ontem"] = df["nat_demand"].shift(24)
df["mesma_hora_semana"] = df["nat_demand"].shift(168)

df["ratio_24h"] = df["lag_1h"] / (df["media_24h"] + 1e-5)
df["ratio_168h"] = df["lag_1h"] / (df["media_168h"] + 1e-5)
df["ratio_672h"] = df["lag_1h"] / (df["media_672h"] + 1e-5)
...
df["diff_48h"] = df["lag_1h"] - df["lag_48h"]
df["diff_168h"] = df["lag_1h"] - df["lag_168h"]
df["diff_336h"] = df["lag_1h"] - df["lag_336h"]
...
calendar_features = [
    ...
    "pico_hora", "manha", "tarde", "noite", "madrugada",
]
...
features = weather_features + calendar_features + lag_features + rolling_features + extra_features
...
for nome_horizonte, passos in horizontes.items():
    ...
    df_h[f"diff_horizonte_{passos}h"] = df_h["lag_1h"] - df_h[f"lag_horizonte_{passos}h"]
    ...
    features_h = [col for col in features_h if col in df_h.columns]
    ...
    for nome_modelo, model in modelos.items():
        ...
        previsoes_gerais.append(pd.DataFrame({
            "datetime": datas_test,
    ...
    print(f"\nCalculando Ensemble V7 para {nome_horizonte}...")

    pesos = []
    predicoes = []

    for nome_modelo, item in modelos_treinados.items():
        peso = 1 / ((item["rmse"] * (1 + item["mape"] / 100)) + 1e-5)

        if nome_modelo == "ExtraTrees" and passos in [6, 12, 24]:
            peso *= 1.12

        if nome_modelo == "RandomForest" and passos == 1:
            peso *= 1.10
    ...
    y_pred_ensemble = np.maximum(np.average(predicoes, axis=0, weights=pesos), 0)

    mae, rmse, r2, mape, smape, acuracia = calcular_metricas(y_test, y_pred_ensemble)
    ...
    if best_model is not None and best_result["modelo"] == "HistGradientBoosting":
        joblib.dump(
            best_model,
            f"{MODELS_DIR}/best_model_{nome_horizonte}_v7.pkl",
            compress=3,
        )
    else:
        print("Modelo não salvo para evitar arquivo .pkl gigante.")

    joblib.dump(features_h, f"{MODELS_DIR}/features_{nome_horizonte}_v7.pkl", compress=3)

    plt.figure(figsize=(12, 6))
    plt.plot(best_result["y_test"].values[:300], label="Demanda real")
    plt.plot(best_result["y_pred"][:300], label=f"Previsão {best_result['modelo']}", linestyle="--")
    plt.title(f"EnergyAI V7 Híbrido - {nome_horizonte}")
    plt.xlabel("Amostras")
    plt.ylabel("Demanda elétrica")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{DOCS_DIR}/grafico_energyai_v7_{nome_horizonte}.png", dpi=200)
    plt.close()
...
with open(f"{DOCS_DIR}/metricas.txt", "w", encoding="utf-8") as file:
    file.write("MÉTRICAS DOS MODELOS ENERGYAI V7 HÍBRIDO\n")
    file.write("=======================================\n\n")
...
print("\n===== RESUMO FINAL ENERGYAI V7 HÍBRIDO =====")
